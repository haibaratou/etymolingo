...
### ★ 消してよいかの判断は `app/index.html` が決める

**入口ページからリンクされているものが、消してはいけないもの。** それが定義。
リンクされていないファイルは試作・残骸なので、消してよい。

消す前に必ず入口を見ること:

    grep -o 'href="[^"]*"' app/index.html

作業の途中で「全部消して」と言われても、**入口からリンクされているものは別**。
矛盾していると感じたら、そこで手を止めて確認する。
別の場面の発言(「どうせ動いてない」等)を確認の代わりにしてはいけない。

実際にやらかした: 入口がリンクしていたエティモペディア(プロダクト2)を、
試作ゲームについての発言を根拠に消した。git から復元できたが、
取り返しのつかない操作の前に都合よく解釈したのが問題だった。
