...
配信先リポジトリの名前を変えると、GitHub Pages のURLが変わる
(`<アカウント>.github.io/<新しい名前>/app/index.html`)。
リポジトリのURLは GitHub が転送してくれるが、**Pages のURLは転送されない**。

画面の中にURLを直書きしていないので、画面側の直しは要らない。
共有用URLは実行時に `location.href` から組み立てている。

手元のクローンの向き先だけ直す:

    cd <配信先クローン> && git remote set-url origin <新しいURL>
