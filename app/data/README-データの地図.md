...
エティモン関係は **GitHub に4つ**ある(ほかに FGO 等の別プロジェクトが同じ持ち主の下にある)。
...
`Wild Wordopia` という名前自体は、古い開発名として2か所に残っている。
語源データベースの元になった `docs/WildWordopia PIE.xlsx` と、
配信先の旧作の試作(`app/old/index-caplog-legacy.html`。カピバラGO型の元になったもの)。
どちらも上のリポジトリとは別物。
